...
**Hedef:** Active Directory Keşfi, SMB Anonim Erişim Suistimali, GPP (Group Policy Preferences) cpassword Deşifresi ve Kerberoasting ile Privelege Escalation.
...
![SMBMap Çıktısı](/assets/img/active/4.jpeg)

> **Sonuç:** Anonim (kimlik doğrulamasız) erişimle **`Replication`** paylaşımına **`READ ONLY`** (Sadece Okuma) yetkimiz olduğunu görüyoruz.
...
![Policies Dizinleri](/assets/img/active/5.jpeg)

**Adım 2: Groups.xml Keşfi**
`MACHINE\Preferences\Groups` dizinine ulaştığımızda altın madenini buluyoruz: **`Groups.xml`**. Bu dosyayı okuduğumuzda, **`active.htb\SVC_TGS`** kullanıcısı için oluşturulmuş bir parola ilkesi görüyoruz. Ancak parola açık metin değil, `cpassword` parametresi altında AES-256 ile şifrelenmiş olarak tutuluyor.
...
![Hashcat](/assets/img/active/12.jpeg)
...
Artık Domain Admin yetkilerine sahibiz. Hedef sisteme tam yetkili olarak bağlanıp son bayrağı alıyoruz.

**Adım 1: Sisteme Bağlantı (Wmiexec)**
Yine Impacket araç setinden `wmiexec.py` kullanarak Administrator kimlik bilgileriyle Domain Controller üzerinde yetkili bir shell elde ediyoruz.
![Wmiexec Shell](/assets/img/active/winrm.jpeg)

**Adım 2: Root Bayrağı**
Doğrudan `C:\Users\Administrator\Desktop` dizinine giderek **root.txt** dosyasını okuyoruz ve makineyi bitiriyoruz.
